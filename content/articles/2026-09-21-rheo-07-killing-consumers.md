...
This is part 7 of [Rheo](https://thanos.github.io/series/rheo/). Messaging that cannot survive a killed worker is a happy-path script with a supervision tree glued on.

The demo I actually trust is rude:
...
Those two sentences have to be true without a human running `Rheo.replay/3`. Replay is for “we shipped a bad handler and need the history again.” Expiry is for “the process vanished.” Mixing them up trains operators to treat every crash as an incident.
...
Lease expiry *is* the crash recovery path. A flaky test here is a flaky production story.
...
## Kill it in the demo, then in the test, then in staging
...
Next: the reason I kept the events around while I was busy killing processes.
